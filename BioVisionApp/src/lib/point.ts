...
class PointTool extends FabricTool {
  ...
  onMouseDown(o: any) {
    ...
    let pointNum = canvas.getObjects().filter(obj => obj.type === 'circle').length + 1;
    let text = new fabric.Text(pointNum.toString(), {
      left: this.currentStartX + 10,
      top: this.currentStartY - 10,
      fontSize: 10,
      selectable: false,
      evented: false,
    });
    ...
    if (_clicked === 0) {
      canvas.add(this.currentCircle, text);
      canvas.renderAll();
    }
  }
